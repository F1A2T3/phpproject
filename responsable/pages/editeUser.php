...
if (isset($_POST['submit'])) {
   ...
   if(isset($_FILES['img'])&& $_FILES['img']['error'] === UPLOAD_ERR_OK){ 
    ...
    if($etat=="Inscrit"&&$data['état_inscription']!="Inscrit"){
      $requete = "UPDATE equipe SET nbr_joueurs=nbr_joueurs+1 WHERE Nom_equipe = ?";
      $reponse = $bdd->prepare($requete);
      $reponse->execute([$equipe]);
    }else{
      if($data['état_inscription']=="Inscrit"&&$etat!="Inscrit"){
         $requete = "UPDATE equipe SET nbr_joueurs=nbr_joueurs-1 WHERE Nom_equipe = ?";
         $reponse = $bdd->prepare($requete);
         $reponse->execute([$equipe]);
      }
    ...
    <section id="content">
        <main>
         ...
         <div class="clnext" style="padding-left: 0px;">
            ...
            <select name="etat">
                <option value="Attente_Respo" <?php if($data['état_inscription']=="Attente_Respo"){echo "selected";}?>>Attente responsable</option>
                <option value="Inscrit"<?php if($data['état_inscription']=="Inscrit"){echo "selected";}?>>Inscrit</option>
                <option value="Attente_Capit"<?php if($data['état_inscription']=="Attente_Capit"){echo "selected";}?>>Attente Capitaine</option>
                <option value="Refus_Respo"<?php if($data['état_inscription']=="Refus_Respo"){echo "selected";}?>>Refusé par responsable</option>
                <option value="Refus_Capit"<?php if($data['état_inscription']=="Refus_Capit"){echo "selected";}?>>Refusé par capitaine</option>
            </select><br>
            <?php
               if($data['Nom_equipe']){
                  if($data['CIN']!=$data1[0]){
            ...
            <select id="chEq" name="selectChEq">
               ...
               <div class="input-group">
                  <input type="text" name="equipeName" placeholder="Nom d'equipe" id="crEq"value="<?php echo $data['Nom_equipe']; ?>"><?php }}else{
                      $sql='SELECT * from equipe'; 
                      $reponse=$bdd->prepare($sql);
                      $reponse->execute();
                     if($reponse->rowCount()>0){
                        echo "<label>Changer l'équipe de l'utilisateur</label><select id=\"chEq\" name=\"selectChEq\"\">";
                        while($donnees = $reponse->fetch()){
                          if($donnees[2] != 0){
                              echo "<option value=".$donnees[0].">".$donnees[0]."</option>";
                            } 
