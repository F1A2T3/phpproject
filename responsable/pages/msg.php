...
<?php
                if(isset($_POST['Accept'])){
                    ...
                    if(!is_null($data['equipe2'])){
                      ...
                      if ($result > 0) {
                      echo '<script>';
                      echo 'if(confirm("Les deux équipes participent déjà à '.$result.' autres matchs prévus au cours de la semaine incluant la date saisie par vous.Vous voulez continuer?")) {';
                      ...
                      echo '</script>';
                        ...
                        if($data1[2] != $data2[0]){
                            echo '<script>';
                            echo 'if(confirm("Voulez-vous qu\'une équipe des '.$data1[2].' joue avec une équipe des '.$data2[0].'?")) {';
                            echo 'window.location.href = "msgAccept.php?id='.$id.'";'; // suivre le lien
                            echo '}'; // sinon, rester sur la même page
                            echo '</script>';
                          }
                       ...
                       AND mc.id_terrain = (SELECT id_terrain FROM matchclub WHERE id_match = :id)
                       AND WEEK(mc.temp_debut) = WEEK((SELECT temp_debut FROM matchclub WHERE id_match = :id))
                       AND (mc.equipe1 = (SELECT equipe1 FROM matchclub WHERE id_match = :id)OR mc.equipe2 = (SELECT equipe1 FROM matchclub WHERE id_match = :id)) AND mc.statut = :statut
                       ');
                       $check1->execute(array(':id' => $id,':statut' => $etat));
                       $result1 = $check1->rowCount();
                       if ($result1 > 0) {
                       echo '<script>';
                       echo 'if(confirm("L\'équipe participe déjà à '.$result1.' autres matchs prévus au cours de la semaine incluant la date saisie par vous.Vous voulez continuer?")) {';
                       echo 'window.location.href = "msgAccept.php?id='.$id.'";'; // suivre le lien
                       echo '}else{';
                       echo  'window.location.href = "msg.php";'; // sinon, rester sur la même page
                       echo '}';
                       echo '</script>';
                     }
                     else{header("Location: msgAccept.php?id=".$id);
                       exit();
                      }
                    }
                }
